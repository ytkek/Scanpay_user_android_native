package ths.ScanPay_UserV5;

public class ApiUrl {

    public static String Domain = "https://www.myscanpay.com/V5/mobile_native_api/";
    public static String PicDomain = "https://www.myscanpay.com/";

    public static String FindMerchantApi = "GetAllMerchantList.aspx";
    public static String FindMessageListApi = "GetUserMessageList.aspx";
    public static String GetUserProfileListApi = "GetMyProfileList.aspx";

    public static String PostUserProfile_Name_Api = "PostUserProfile_Name.aspx";
    public static String PostUserProfile_OldPassword_Api = "PostUserProfile_OldPassword.aspx";
    public static String PostUserProfile_NewPassword_Api = "PostUserProfile_NewPassword.aspx";
    public static String PostUserProfile_MobileNumber_Api = "PostUserProfile_MobileNumber.aspx";
    public static String PostUserProfile_Gender_Api = "PostUserProfile_Gender.aspx";
    public static String PostUserProfile_NickName_Api = "PostUserProfile_NickName.aspx";
    public static String PostUserProfile_Remarks_Api = "PostUserProfile_Remarks.aspx";
    public static String PostUserProfile_Dob_Api = "PostUserProfile_Dob.aspx";
    public static String PostUserProfile_Pin_Api = "PostUserProfile_Pin.aspx";
    public static String PostUserProfile_Recover_Pin_Api="Recover_Pin.aspx";

    public static String GetAllDiscoveryListApi = "GetAllDiscoveryList.aspx";

    public static String PostSignUp_Validate_LoginID_Api = "PostSignUp_Validate_LoginID.aspx";
    public static String PostSignUp_Send_OTP_Api = "PostSignUp_Send_OTP.aspx";
    public static String PostSignUp_Update_Password_Api = "PostSignUp_Update_App_Password.aspx";
    public static String PostSignUp_Update_User_Info_Api = "PostSignUp_Update_User_Info.aspx";
    public static String PostSignUp_Update_Payment_Info_Api = "PostSignUp_Update_Payment_Info.aspx";
    public static String PostSignUp_Update_Confirm_Api = "PostSignUp_Update_Confirm.aspx";
    public static String PostSignUp_Update_MemberList_Api = "PostSignUp_Update_MemberList.aspx";

    public static String PostLogin_Validate_LoginID_Api = "PostLogin_Validate_LoginID.aspx";
    public static String PostLogin_Recover_LoginID_Password_Api="Recover_loginid_password.aspx";
    public static String PostLogout_Update_LoginID_Api = "PostLogout_Update_Status.aspx";

    public static String PostPay_Validate_QRExpired_Api = "PostPay_Validate_QRExpired.aspx";
    public static String PostPay_CreditBalance_Api = "PostPay_CreditBalance.aspx";
    public static String PostPay_MerchantInfo_Api = "PostPay_MerchantInfo.aspx";
    public static String PostPay_CheckDailyExpLimit_Api = "PostPay_CheckDailyExpLimit.aspx";
    public static String PostPay_Validate_PinNumber_Api = "PostPay_Validate_PinNumber.aspx";
    public static String PostPay_Validate_OTP_Api = "PostPay_Validate_OTP.aspx";
    public static String PostPay_Send_OTP_Api = "PostPay_Send_OTP.aspx";
    public static String PostPay_Save_OTP_Api = "PostPay_Save_OTP.aspx";
    public static String PostPay_Confirm_Pay_Api = "PostPay_Confirm_Pay.aspx";
    public static String PostPay_DailyExp_Api = "PostPay_DailyExp.aspx";
    public static String PostPay_MonthlyExp_Api = "PostPay_MonthlyExp.aspx";
    public static String PostPay_Duplicate_Transaction_Alert_Api = "PostPay_Duplicate_Transaction_Alert.aspx";

    public static String PostTopUp_MerchantInfo_Api = "PostTopUp_MerchantInfo.aspx";

    public static String PostBalance_TotalCredit_Api = "GetBalance_TotalCredit.aspx";
    public static String PostBalance_DailyLimit_Api = "GetBalance_DailyLimit.aspx";
    public static String PostBalance_DailyExp_Api = "GetBalance_DailyExp.aspx";
    public static String PostBalance_CurrentDate_Statement_Api = "GetBalance_CurrentDate_Statement.aspx";
    public static String PostBalance_SearchDate_Statement_Api = "GetBalance_SearchDate_Statement.aspx";

    public static String PostVerification_Validate_LoginID_Api = "PostVerification_Validate_LoginID.aspx";
    public static String PostVerification_Validate_Email_Api = "PostVerification_Validate_Email.aspx";
    public static String PostVerification_Send_OTP_Api = "PostVerification_Send_OTP.aspx";
    public static String PostVerification_Save_OTP_Api = "PostVerification_Save_OTP.aspx";


    public static String PostApp_Success_Message_Api = "PostApp_Success_Message.aspx";
    public static String PostApp_Error_Message_Api = "PostApp_Error_Message.aspx";
}
