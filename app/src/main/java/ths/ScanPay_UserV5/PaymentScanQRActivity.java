...
import ths.ScanPay_UserV5.PostFunction.PostPay_CreditBalance_Task;
import ths.ScanPay_UserV5.PostFunction.PostPay_DailyExp_Task;
import ths.ScanPay_UserV5.PostFunction.PostPay_Duplicate_Transaction_Alert_Task;
        ...
        if(result.getContents() == null) {
            ...
            try{
                ...
                if (arrayString.length <=0 || arrayString.length >3)
                {
                    ...
                    {
                    new PostPay_Get_OTP_Task(PaymentScanQRActivityactivity).execute(MainActivity.LoginID,MainActivity.LoginID,MainActivity.Password);
                    new PostPay_DailyExp_Task(this).execute(MainActivity.LoginID,MainActivity.LoginID,MainActivity.Password);
                    new PostPay_MonthlyExp_Task(this).execute(MainActivity.LoginID,MainActivity.LoginID,MainActivity.Password);
                    new PostPay_CreditBalance_Task(this).execute(MainActivity.LoginID,MainActivity.LoginID,MainActivity.Password);
